...
public class Flight {
	...
	public void updatePassengerNum(){
		for (BoardingPass f : boardingPassList){
			if(f.getPassenger().getTicketClass()=='O'){
				economyClassNum++;
			}
			else if(f.getPassenger().getTicketClass()=='B'){
				businessClassNum++;
			}
			else if(f.getPassenger().getTicketClass()=='F'){
				firstClassNum++;
			}
		}
	}
	
	public void printPassengerNumByTicketClass(){
		updatePassengerNum();
		System.out.println("Number of Economy Class Passengers : "+economyClassNum);
		System.out.println("Number of Business Class Passengers : "+businessClassNum);
		System.out.println("Number of First Class Passengers : "+firstClassNum);
	}
	
	/*public int first(){
		for (BoardingPass f : boardingPassList){
			if(f.getPassenger().getTicketClass()=='F'){
				firstClassNum++;
			}
		}
		return firstClassNum;
