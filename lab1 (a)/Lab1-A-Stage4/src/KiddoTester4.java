...
public class KiddoTester4 {
	...
	public static void totalStudentEachClass(){
		...
		for(Student ss : kiddoList){
			if(ss.getKclass().getType() == "Kiddo Art"){
				art ++ ;
			}
			else if(ss.getKclass().getType() == "Kiddo Science"){
				if(ss.getKclass().getLevel() == "Introductory"){
					introductory ++ ;
				}
				else if(ss.getKclass().getLevel() == "Discovery"){
					discovery ++ ;
				}
				else if(ss.getKclass().getLevel() == "Experimental"){
					experimental ++ ;
				}
			}
		}
